...
if ($controller) {
    if (count($pathChunks) === 2) {
        ...
        call_user_func_array(array($controller, $method), [$id,
            array_merge($_POST, $_GET, $_FILES, $_SERVER, $_COOKIE)
        ]);
        ...
        if (is_numeric($chunk)) {
            $id = (int) $chunk;
            $controller->getById($id, array_merge($_POST, $_GET, $_FILES, $_SERVER, $_COOKIE));
        } else {
            $method = $chunk;
        ...
        $controller->index(array_merge($_POST, $_GET, $_FILES, $_SERVER, $_COOKIE));
    }
}
